...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        // COMPLETAR
        Arista numV[] = gRM.kruskal();
        
        // PASO 2. Si kruskal devuelve null, devolver -1.0
        // COMPLETAR
        if(numV == null){return -1.0;}
        adyKruskal = new TablaHash<>(MAX_MUNICIPIOS);
        ...
        for(int i = 0; i < numV.length; i++) {
            Arista a = numV[i];
            res += a.getPeso();
            Municipio m = verticesAMunicipios.recuperar(a.getOrigen());
            ListaConPI<Municipio> lm = adyKruskal.recuperar(m);
            if(lm == null) { lm = new LEGListaConPI<Municipio>(); }
            lm.insertar(verticesAMunicipios.recuperar(a.getDestino()));
            Municipio m1 = verticesAMunicipios.recuperar(a.getDestino());
            ListaConPI<Municipio> lm1 = adyKruskal.recuperar(m1);
            if(lm1 == null) {lm1 = new LEGListaConPI<Municipio>(); }
            lm1.insertar(verticesAMunicipios.recuperar(a.getOrigen()));
            adyKruskal.insertar(m1,lm1);
        }
